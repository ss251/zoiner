...
2. If user requests token creation: Extract name/symbol or suggest based on image
3. If user needs help: Provide encouraging, art-focused guidance
4. If unclear intent: Ask clarifying questions about their creative vision

RESPONSE FORMAT:
Always respond with JSON:
{
  "message": "your encouraging response text",
  "action": "create_token" | "clarify" | "help" | "encourage" | "celebrate",
  "suggested_name": "if action is create_token",
  "suggested_symbol": "if action is create_token", 
...
export class ZoinerAgentService {
  ...
  async processInteraction(cast: FarcasterCast): Promise<void> {
    ...
    try {
      ...
      // 3. AI decision
      const decision = await this.makeDecision(cast, imageAnalysis, userContext);

      // 4. Execute decision
      await this.executeDecision(decision, cast, imageAnalysis);
